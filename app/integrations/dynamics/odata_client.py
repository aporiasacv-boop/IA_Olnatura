...
Cliente HTTP desacoplado que consume entidades OData autenticadas con OAuth.
"""
...
from app.integrations.dynamics.exceptions import (
...
class ODataClientConfig:
    """Parámetros de conexión OData configurables."""
...
class DynamicsODataClient:
    """
    Cliente OData para Dynamics 365 F&O.

    No almacena datos; solo consulta entidades vía HTTP.
    """
    ...
    def query_entity(self, entity_name: str, top: int = 1) -> dict[str, Any]:
        """
        Consulta una entidad OData de Dynamics 365 F&O.

        Args:
            entity_name: Nombre de la entidad (ej. Companies, CustomersV3).
            top: Límite de registros ($top).

        Returns:
            Cuerpo JSON de la respuesta OData.

        Raises:
            DynamicsConnectionError: Error de red o timeout.
            DynamicsODataError: Respuesta HTTP no exitosa o JSON inválido.
        """
    ...
    def fetch_all_entity(self, entity_name: str, page_size: int = 100) -> list[dict[str, Any]]:
        """
        Extrae todos los registros de una entidad OData con paginación.

        Sigue @odata.nextLink hasta agotar páginas.

        Args:
            entity_name: Nombre de la entidad (ej. CustomersV3).
            page_size: Registros por página ($top).

        Returns:
            Lista acumulada de registros OData.
        """
    ...
    def _request_odata(
        ...
        """Ejecuta una petición GET OData autenticada."""
        ...
        try:
            ...
            raise DynamicsConnectionError(
                f"No se pudo conectar a Dynamics 365 OData: {exc}"
            ) from exc

        if response.status_code != 200:
            raise DynamicsODataError(
                f"Error OData {response.status_code}: {response.text}",
        ...
        if "value" not in data and "@odata.nextLink" not in data:
            raise DynamicsODataError(
                "La respuesta OData no contiene el campo 'value' esperado"
            )
