...
def test_index_file_loads_splits_and_indexes() -> None:
    """Verifica pipeline de indexación completo."""
    ...
    chunks = [
        ...
        Document(page_content="chunk 2", metadata={"source": "doc.pdf"}),
    ]
...
def test_query_retrieves_context_and_generates_answer() -> None:
    """Verifica consulta RAG con recuperación y generación LLM."""
    vector_store = MagicMock()
    vector_store.similarity_search.return_value = [
        Document(page_content="Política de ventas Q2", metadata={"source": "manual.docx"}),
    ]
    llm = MagicMock()
...
def test_query_returns_message_when_no_documents() -> None:
    """Verifica respuesta cuando no hay documentos indexados."""
    vector_store = MagicMock()
    vector_store.similarity_search.return_value = []
    llm = MagicMock()
