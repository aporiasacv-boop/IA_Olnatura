...
def test_with_retry_succeeds_after_failures() -> None:
    operation = MagicMock(side_effect=[DynamicsConnectionError('timeout'), DynamicsConnectionError('timeout'), 'ok'])
    result = with_retry(operation, max_retries=3, base_delay=0.01)
    assert result == 'ok'
    assert operation.call_count == 3

def test_with_retry_raises_after_max_attempts() -> None:
    operation = MagicMock(side_effect=DynamicsConnectionError('timeout'))
