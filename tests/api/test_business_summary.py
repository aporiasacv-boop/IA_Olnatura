"""
Pruebas del endpoint POST /ai/business-summary.
"""
...
def test_post_business_summary_returns_interpretation() -> None:
    """Verifica respuesta exitosa con interpretación empresarial."""
    mock_service = MagicMock(spec=BusinessInterpretationService)
    mock_service.generate_summary.return_value = (
        "Las ventas del mes reflejan un volumen significativo con una base de clientes activa."
    )
    ...
    try:
        with TestClient(app) as client:
            response = client.post(
                "/ai/business-summary",
                json={"ventas_mes": 125000, "clientes": 230},
            )
    finally:
...
def test_post_business_summary_returns_503_on_ollama_error() -> None:
    ...
    mock_service = MagicMock(spec=BusinessInterpretationService)
    mock_service.generate_summary.side_effect = OllamaConnectionError(
...
def test_post_business_summary_validates_negative_values() -> None:
    ...
    with TestClient(app) as client:
        response = client.post(
            "/ai/business-summary",
            json={"ventas_mes": -1, "clientes": 230},
        )
