...
def test_post_documents_index_success() -> None:
    ...
    mock_service.index_file.return_value = IndexResult(filename='informe.pdf', chunks_indexed=5)
    app.dependency_overrides[get_rag_service] = lambda: mock_service
    try:
        with TestClient(app) as client:
            response = client.post('/documents/index', files={'file': ('informe.pdf', BytesIO(b'%PDF-1.4 fake'), 'application/pdf')})
    finally:
...
def test_post_documents_index_rejects_unsupported_format() -> None:
    with TestClient(app) as client:
        response = client.post('/documents/index', files={'file': ('notas.txt', BytesIO(b'hola'), 'text/plain')})
    assert response.status_code == 400
    assert 'Formato no soportado' in response.json()['detail']

def test_post_documents_query_success() -> None:
    mock_service = MagicMock(spec=RAGService)
    mock_service.query.return_value = QueryResult(answer='La política de ventas establece metas trimestrales.', sources=[SourceChunk(content='fragmento', metadata={'source': 'manual.docx'})])
    app.dependency_overrides[get_rag_service] = lambda: mock_service
    try:
        with TestClient(app) as client:
            response = client.post('/documents/query', json={'question': '¿Cuál es la política de ventas?'})
    finally:
        app.dependency_overrides.clear()
    assert response.status_code == 200
    data = response.json()
...
def test_post_documents_query_returns_503_on_llm_error() -> None:
    ...
    try:
        with TestClient(app) as client:
            response = client.post('/documents/query', json={'question': '¿Algo?'})
    finally:
        app.dependency_overrides.clear()
