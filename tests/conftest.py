...
def mock_db_session() -> MagicMock:
    """Sesión de BD simulada con respuesta exitosa a SELECT 1."""
...
def client_with_db(mock_db_session: MagicMock) -> Generator[TestClient, None, None]:
    """
    Cliente de prueba con la dependencia de BD sobreescrita.

    Evita requerir PostgreSQL activo durante las pruebas unitarias.
    """
...
def analytics_db() -> Generator[Session, None, None]:
    """SQLite en memoria con datos de ventas y clientes para analytics."""
    ...
    session.add_all(
        [
            ...
            Cliente(dynamics_id="C003", nombre="Gamma Ltd"),
        ]
    )
    session.add_all(
        [
