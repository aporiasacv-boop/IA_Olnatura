...
def ollama_config() -> OllamaClientConfig:
    return OllamaClientConfig(base_url='http://localhost:11434', model='llama3.2', timeout=30.0)

def test_generate_returns_model_response(ollama_config: OllamaClientConfig) -> None:
    http_client = MagicMock()
    http_response = MagicMock()
    http_response.status_code = 200
    http_response.json.return_value = {'model': 'llama3.2', 'response': '¡Hola! ¿En qué puedo ayudarte?', 'done': True}
    ...
    http_client.post.assert_called_once_with('http://localhost:11434/api/generate', json={'model': 'llama3.2', 'prompt': 'Hola', 'stream': False})
...
def test_generate_raises_connection_error_on_network_failure(ollama_config: OllamaClientConfig) -> None:
    http_client = MagicMock()
...
def test_generate_raises_api_error_when_response_missing(ollama_config: OllamaClientConfig) -> None:
    http_client = MagicMock()
    http_response = MagicMock()
    http_response.status_code = 200
