"""
Pruebas del cliente OData Dynamics 365 F&O.
"""
...
def test_query_entity_returns_odata_response(
    odata_config: ODataClientConfig,
    oauth_provider: MagicMock,
) -> None:
    """Verifica consulta exitosa a entidad OData."""
    ...
    http_client.get.assert_called_once_with(
        "https://example.operations.dynamics.com/data/Companies",
        headers={
...
def test_ping_delegates_to_health_entity(
    odata_config: ODataClientConfig,
    oauth_provider: MagicMock,
) -> None:
    """Verifica que ping consulta la entidad de prueba configurada."""
    client = DynamicsODataClient(
        config=odata_config,
        oauth_provider=oauth_provider,
        http_client=MagicMock(),
...
def test_query_entity_raises_odata_error_on_http_failure(
    odata_config: ODataClientConfig,
    oauth_provider: MagicMock,
) -> None:
    """Verifica error OData ante respuesta HTTP 500."""
    http_client = MagicMock()
    http_response = MagicMock()
...
def test_query_entity_raises_connection_error_on_network_failure(
    odata_config: ODataClientConfig,
    oauth_provider: MagicMock,
) -> None:
    """Verifica error de conexión ante timeout."""
    http_client = MagicMock()
...
def test_query_entity_raises_odata_error_when_value_missing(
    odata_config: ODataClientConfig,
    oauth_provider: MagicMock,
) -> None:
    """Verifica error cuando la respuesta OData no incluye 'value'."""
    http_client = MagicMock()
    http_response = MagicMock()
    http_response.status_code = 200
