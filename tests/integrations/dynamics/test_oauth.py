"""
Pruebas del proveedor OAuth Azure AD.
"""
...
def oauth_config() -> OAuthConfig:
    return OAuthConfig(
        ...
        scope="https://example.operations.dynamics.com/.default",
...
def test_get_access_token_returns_bearer_token(oauth_config: OAuthConfig) -> None:
    """Verifica obtención exitosa de token OAuth."""
...
def test_get_access_token_raises_auth_error_on_invalid_response(
    oauth_config: OAuthConfig,
) -> None:
    """Verifica error de autenticación cuando OAuth retorna 401."""
    http_client = MagicMock()
    http_response = MagicMock()
...
def test_get_access_token_raises_connection_error_on_network_failure(
    oauth_config: OAuthConfig,
) -> None:
...
def test_get_access_token_uses_cache_on_second_call(oauth_config: OAuthConfig) -> None:
    """Verifica que el token se cachea y no se solicita dos veces."""
    http_client = MagicMock()
    http_response = MagicMock()
    http_response.status_code = 200
    http_response.json.return_value = {
