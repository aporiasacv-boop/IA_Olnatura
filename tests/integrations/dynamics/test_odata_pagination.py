...
def test_fetch_all_entity_follows_next_link() -> None:
    ...
    first_response.json.return_value = {'value': [{'CustomerAccount': 'C001'}], '@odata.nextLink': 'https://example.operations.dynamics.com/data/CustomersV3?$skiptoken=abc'}
    ...
    client = DynamicsODataClient(config=ODataClientConfig(base_url='https://example.operations.dynamics.com/data', health_entity='Companies'), oauth_provider=oauth, http_client=http_client)
